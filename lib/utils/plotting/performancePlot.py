...
#jdflist = [df1, df2, df3, df4, df5, df6, df7, df8]
...
for key in df1.keys():
    ...
    headers = ['1', '2', '4', '8']
    for header in headers:
        plotdata.append(plotDF['1'].mean()/plotDF[header].mean()/float(header))

    plotDF = builddfSpeedUp(frames)
    plt.plot(headers, plotdata, label="Matirx Size = "+key +'x'+key)
    #ax = sns.catplot(data = plotDF, kind='violin')
...
plt.grid()
#plt.title("Matirx Size = "+key +'x'+key)
plt.savefig("EfficiencySize.png", dpi=900)
plt.tight_layout()
plt.close()
